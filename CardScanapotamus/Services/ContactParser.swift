...
struct ContactParser {
    static func parse(lines: [String]) -> ScannedCard {
        ...
        for line in unclaimedLines {
            if !nameAssigned && looksLikeName(line) {
                card.fullName = line
                nameAssigned = true
    ...
    private static func extractPhone(from text: String) -> String? {
        let stripped = text.replacingOccurrences(of: "[^0-9+()\\-\\s.]", with: "", options: .regularExpression)
        let digitCount = stripped.filter(\.isNumber).count
        if digitCount >= 7 && digitCount <= 15 {
            // Check that the original line looks phone-like
